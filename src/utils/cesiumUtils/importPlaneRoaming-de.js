    ...
    this.viewer.clock.onTick.addEventListener((result) => {
      if (_this && _this.entity) {
        ...
        if (tarentity.id && tarentity.id === 'airPlane') {
          ...
          const nowText = `${$t('plane longitude:')}${lon.toFixed(2)}° 纬度：${lat.toFixed(
            2
          )}° 高度：${height.toFixed(2)}m, 速度：${tarentity.speed.toFixed(2)}m/s`
          // 不同时更新
    ...
    const defaultConf = {
      ...
      polyline: {
        positions: new Cesium.CallbackProperty((time, result) => {
          ...
          if (
            pathArr.length > 50 &&
            _this.getLineDis(_this.position, [lon1, lat1, height1]) < 1000
          ) {
