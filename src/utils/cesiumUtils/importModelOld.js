    ...
    this.renderFunc = this.viewer.clock.onTick.addEventListener((result) => {
      if (_this && _this.entity) {
        ...
        if (tarentity.id && tarentity.id === 'airPlane') {
          ...
          const nowText = `${$t('plane longitude:')}${lon.toFixed(2)}° ${$t(
            'latitude:'
          )}${lat.toFixed(2)}° ${$t('height:')}${height.toFixed(2)}m, ${$t(
            'speed:'
          )}${tarentity.speed.toFixed(2)}m/s`
          // update async
    ...
    this.wave = this.viewer.entities.add({
      ...
      polyline: {
        positions: new Cesium.CallbackProperty((time, result) => {
          ...
          const latestEntityPosition = this.getNearestEntityPosition(
            entities,
            time,
            lon1,
            lat1,
