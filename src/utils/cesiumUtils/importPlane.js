...
export default class ImportPlane {
  ...
  InitPlane() {
    ...
    this.preUpdate = viewer.scene.preUpdate.addEventListener((scene, time) => {
      ...
      const A = Cesium.Cartesian3.fromDegrees(
        _this.arr[_this.i - 1][0],
        _this.arr[_this.i - 1][1],
        _this.arr[_this.i - 1][2]
      )
      ...
      cartesian3 = Cesium.Matrix4.multiplyByPoint(_this.entity.modelMatrix, speedVector, cartesian3)
    ...
    setTimeout(() => {
      camera.flyTo({
        destination: Cesium.Cartesian3.fromDegrees(this.position[0], this.position[1] - 0.1, 10000),
        orientation: {
          // heading
      ...
      polyline: {
        positions: new Cesium.CallbackProperty((time, result) => {
          ...
          return Cesium.Cartesian3.fromDegreesArrayHeights(
            [lon1, lat1, height1, ...latestEntityPosition],
            Cesium.Ellipsoid.WGS84,
            result
          )
        }, false),
    ...
    const defaultConf = {
      ...
      polyline: {
        positions: new Cesium.CallbackProperty((time, result) => {
          ...
          return Cesium.Cartesian3.fromDegreesArrayHeights(
            [lon1, lat1, height1, lon2, lat2, height2],
            Cesium.Ellipsoid.WGS84,
            result
          )
        }, false),
  ...
  getNearestEntityPosition(poss, lon1, lat1, height1) {
    ...
    poss.forEach((item) => {
      ...
      const distance = this.getLineDis(
        [lon1, lat1, height1 <= 0 ? 0 : height1],
        [lon2, lat2, height2 <= 0 ? 0 : height2]
      )
